...
FFTViewerComponent::~FFTViewerComponent() { shutdownOpenGL(); }
...
void FFTViewerComponent::render()
{
    ...
    for (std::size_t index = 0; index < componentsData.size(); ++index)
    {
        ...
        color->set(viewColors[3 * index], viewColors[3 * index + 1],
                   viewColors[3 * index + 2], .9f);

        componentsData[index].display(data, index, sampling_rate, process,
...
{
    std::string vertexShader = "attribute vec3 position;\n"
                               "\n"
                               "uniform mat4 MVP;\n"
                               "\n"
                               "void main()\n"
    ...
    std::string fragmentShader = "uniform vec4 color;\n"
                                 "void main()\n"
                                 "{\n"
                                 "  gl_FragColor = color;\n"
                                 "}";
        ...
        newShader->link())
    {
        ...
        statusText = "GLSL: v" +
                     ::juce::String(
                         ::juce::OpenGLShaderProgram::getLanguageVersion(), 2);
    }
    else
    {
    ...
    transformationMatrix =
        glm::ortho(-1.f, 1.f, -1.f, 1.f, 10.f, -10.f) *
    ...
    position.reset(
        new ::juce::OpenGLShaderProgram::Attribute(*shader, "position"));
...
void FFTViewerComponent::display_grid()
{
    ...
    openGLContext.extensions.glBufferData(
        GL_ARRAY_BUFFER,
        static_cast<GLsizeiptr>(grid_data.size() * sizeof(float)),
        grid_data.data(), GL_STATIC_DRAW);

    openGLContext.extensions.glEnableVertexAttribArray(position->attributeID);
    openGLContext.extensions.glVertexAttribPointer(position->attributeID, 3,
...
FFTViewerComponent::Component::Component(FFT<double>& fft,
...
void FFTViewerComponent::Component::smooth_display()
{
    ...
    {
        display_data[3 * index + 1] =
            (temp_display_data[index - 1] + temp_display_data[index] +
             temp_display_data[index + 1]) /
            3;
    }
    display_data[3 * (temp_display_data.size() - 1) + 1] =
        temp_display_data[temp_display_data.size() - 1];
}

void FFTViewerComponent::Component::display(const std::vector<double>& data,
                                            int depth, int sampling_rate,
                                            bool process, GLuint positionID)
{
    ...
    {
        double memory_rate =
            std::exp(-0.3 * data.size() / sampling_rate); // 300ms release time
        fft.set_size(slice_size);
        fft.process(data.data(), slice_size);
        fft.get_amp(amp_data);

        auto ratio = double(slice_size) / sampling_rate;
        ...
        if (amp_data_previous.size() != amp_data.size())
        {
            ...
            {
                cumulativeIndices[i] =
                    20 * std::pow(10, i * 3. / (cumulativeIndices.size() - 1)) *
                    ratio;
            }
        }
        ...
        {
            amp_data_previous[i] =
                std::max(amp_data[i], memory_rate * amp_data_previous[i]);
            amp_data_log[i] =
                10 * std::log(amp_data_previous[i]); // amp_data is power
        }
        ...
        for (std::size_t local_id = 0; local_id < cumulativeIndices.size();
             ++local_id)
        {
            display_data[local_id * 3] =
                (local_id * 2. / (cumulativeIndices.size() - 1)) - 1;
            display_data[local_id * 3 + 2] = depth;

            display_data[local_id * 3 + 1] = 0;
            for (std::size_t index = previous_index;
                 index < cumulativeIndices[local_id]; ++index)
            {
                display_data[local_id * 3 + 1] +=
                    (2 * (amp_data_log[index] - min_value) /
                         (max_value - min_value + 1e-10) -
                     1);
            }
            if (cumulativeIndices[local_id] - previous_index)
                display_data[local_id * 3 + 1] /=
                    cumulativeIndices[local_id] - previous_index;
            else
                display_data[local_id * 3 + 1] =
                    display_data[(local_id - 1) * 3 + 1];
    ...
    openGLContext.extensions.glBufferData(
        GL_ARRAY_BUFFER,
        static_cast<GLsizeiptr>(display_data.size() * sizeof(float)),
        display_data.data(), GL_STATIC_DRAW);

    openGLContext.extensions.glEnableVertexAttribArray(positionID);
    openGLContext.extensions.glVertexAttribPointer(positionID, 3, GL_FLOAT,
                                                   GL_FALSE, 0, 0);
