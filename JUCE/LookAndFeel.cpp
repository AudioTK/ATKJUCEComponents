...
void SimpleSliderLookAndFeel::drawRotarySlider(::juce::Graphics& g, int x,
                                               ...
                                               ::juce::Slider& slider)
{
    ...
    if (radius > 12.0f)
    {
        ...
        {
            ::juce::Path filledArc;
            filledArc.addPieSegment(rx, ry, rw, rw, rotaryStartAngle, angle,
                                    thickness);
        ...
        outlineArc.addPieSegment(rx, ry, rw, rw, rotaryStartAngle,
                                 rotaryEndAngle, thickness);
        outlineArc.closeSubPath();

        g.strokePath(outlineArc,
                     ::juce::PathStrokeType(slider.isEnabled()
                                                ? (isMouseOver ? 2.0f : 1.2f)
                                                : 0.3f));
    }
    else
    {
        ...
        p.addLineSegment(::juce::Line<float>(0.0f, 0.0f, 0.0f, -radius),
                         rw * 0.2f);

        g.fillPath(p, ::juce::AffineTransform::rotation(angle).translated(
                          centreX, centreY));
    }
}
...
void DualSliderLookAndFeel::drawRotarySlider(::juce::Graphics& g, int x, int y,
    ...
    if (radius > 12.0f)
    {
...

        {
            ...
            filledArc.addPieSegment(rx, ry, rw, rw,
                                    (rotaryEndAngle + rotaryStartAngle) / 2,
                                    angle, thickness);
            g.fillPath(filledArc);
        }
